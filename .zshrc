...
__conda_setup="$('/Users/wesley/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/Users/wesley/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/Users/wesley/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/Users/wesley/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
alias gchm="git checkout main; gsub"	
alias gr="git rebase"	
...
alias vz="vim ~/.zshrc"	
alias v="vim"	
alias chromedebug='/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome --remote-debugging-port=9222 --user-data-dir=remote-debug-profile'	
...
if [[ "$(hostname)" == "MacBook-Pro.local" ]]; then source ~/.einblick_setup; fi
if [[ "$(hostname)" == "Wesleys-MacBook-Pro-2.local" ]]; then source ~/.einblick_setup; fi
...
if [[ "$(hostname)" == "MacBook-Pro.local" ]]; then source ~/.einblick_setup; fi
if [[ "$(hostname)" == "Wesleys-MacBook-Pro-2.local" ]]; then source ~/.einblick_setup; fi
if [[ "$(hostname)" == "Wesleys-MBP-2.lan" ]]; then source ~/.einblick_setup; fi
